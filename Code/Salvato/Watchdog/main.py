...
def commit_takeover(
    ...
    try:
        for crtc in card.crtcs:
            ...
            # CRTC background becomes #FF00AA.
            atomic_add(
                req,
                crtc.crtc_id,
        ...
        if rc != 0:
            raise err(
                f"{card.path}: "
                "pink atomic commit failed"
            )

        card.pink_active = True
...
def main() -> int:
    ...
    try:
        ...
        if not cards:
            raise RuntimeError(
                "no active KMS displays found"
            )

        print()
        print(
            f"Target background: "
            f"0x{PINK:016x} (#FF00AA)"
        )

        # ---------------------------------------------------------------
        # Acquire every display card first.
        # ---------------------------------------------------------------

        print()
        print("Taking DRM master...")

        acquired: list[CardState] = []

        try:
            for card in cards:
                take_master(card)
                acquired.append(card)

        except Exception:
            for card in reversed(acquired):
                try:
                    return_master(card)
                except Exception:
                    pass
        ...
        try:
            ...
            print()
            print(
                "Forcing all active outputs to #FF00AA..."
            )

            for card in cards:
                commit_takeover(card)
                changed.append(card)

            print(
                f"Pink active for {DURATION:g} seconds."
            )
            ...
            # -----------------------------------------------------------
            # Restore pixels while WE still own KMS.
            # -----------------------------------------------------------

            print(
                "Restoring original display composition..."
            )

            for card in reversed(changed):
                try:
                    commit_restore(card)
                except Exception as exc:
                    print(
            ...
            # -----------------------------------------------------------
            # Only after restoration do we give DRM master back.
            # -----------------------------------------------------------

            print(
                "Returning DRM master..."
            )
